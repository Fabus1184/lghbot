...
from functions import ban, config, suggest, help, set, get, leaderboard, stats, trio, mac, pppkkk, vplan, graph, pyplan

load_dotenv()
TOKEN = os.getenv("DISCORD_TOKEN")
bot = commands.Bot(command_prefix=config.config['prefix'], help_command="help")
...
async def on_command_error(ctx, error):
    if error != TimeoutError:
        await ctx.send(
            "%s an error occurred: %s" % (ctx.message.author.mention, str(error))
        )
