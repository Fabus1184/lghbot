...
def to_lb(id, points, category):
    ...
    # print(points)
    # print(category)
    tmp = leaderboard.search((where('id') == id) &
                             (where('category') == category))
    # print(tmp)
    if tmp:
        leaderboard.update({'points': (int(
            tmp[0]['points'])+int(points))}, (where('id') == id) & (where('category') == category))
    else:
        # print("INSERT")
...
async def lb(ctx):
    ...
    if list:
        ...
        if erster > 1000:
            erster = str(erster/1000).replace(".",
                                              ",").replace(".", ",").split("points")[0] + "k"
        if zweiter > 1000:
            zweiter = str(zweiter/1000).replace(".",
                                                ",").split("points")[0] + "k"
        if dritter > 1000:
            dritter = str(dritter/1000).replace(".",
                                                ",").split("points")[0] + "k"
    ...
    embed = discord.Embed(title="Server Leaderboard", color=0xf1a90f,
...
async def stats(ctx):
    ...
    for x in (leaderboard.search((Query().category == "mac_100") & (Query().id == ctx.author.id))):
        mac100 += str(x['points'])
    for x in (leaderboard.search((Query().category == "mac_1000") & (Query().id == ctx.author.id))):
        mac1000 += str(x['points'])
    for x in (leaderboard.search((Query().category == "mac_10000") & (Query().id == ctx.author.id))):
        mac10000 += str(x['points'])
    for x in (leaderboard.search((Query().category == "trio") & (Query().id == ctx.author.id))):
        trio += str(x['points'])
    for x in (leaderboard.search((Query().category == "pipapo") & (Query().id == ctx.author.id))):
        pipapo += str(x['points'])
    ...
    embed = discord.Embed(title="Personal Stats:", color=0xf1a90f,
                          description="*only games with more than one player are counted*")
    embed.add_field(name="User:", value="%s" %
...
async def settz(ctx, classes, grade):
    try:
        if not (classes and grade):
            await ctx.send("%s, please set classes and grade as shown in %shelp" % (ctx.message.author.mention, ctx.prefix))
            return
        if not db.search(Query().id == ctx.message.author.id):
            #print("NEW ENTRY")
            classes = str(classes).replace(" ", "")
            db.insert({"id": ctx.message.author.id,
                       "classes": str(classes), "grade": (grade)})
...
async def get(ctx):
    try:
        if not db.search(Query().id == ctx.message.author.id):
            ...
            await ctx.send(str("%s\nclasses: `" + str(gett[0]['classes']) + "`\ngrade: `" + str(gett[0]['grade']) + "`") % ctx.author.mention)
    except Exception:
        await ctx.send(str(general_error_message) % ctx.message.author.mention)
...
async def vplan(ctx):
    try:
        classes = (db.search(Query().id == ctx.message.author.id)
                   [0]['classes'])
        # print(classes)
        if not classes:
            await ctx.send("%s, please set your desired classes first!" % ctx.message.author.mention)
            return
        grade = (db.search(Query().id == ctx.message.author.id)[0]['grade'])
        process = Popen(["python3", "pyplan.py", str(
            classes), str(grade)], stdout=PIPE)
        ...
        if len(output.decode("UTF-8")) >= 2000:
            ...
            for x in text.split("\n"):
                if i < 17:
                    ...
                    text2 += "\n"
                i += 1
            await ctx.send("%s your vplan is too long, sending as dm" % ctx.message.author.mention)
            await channel.send("%s, here is your current :calendar_spiral: Vertretungsplan **(%s)**: \n```\n%s```" % (ctx.message.author.mention, datum, text1))
            await channel.send("```\n%s```" % text2)
        else:
            dm = False
            ...
            if dm:
                await ctx.send("%s your vplan is too long, sending as dm" % ctx.message.author.mention)
                await channel.send("%s, here is your current :calendar_spiral: Vertretungsplan **(%s)**: \n```\n%s```" % (ctx.message.author.mention, datum, text))
            else:
                await ctx.send("%s, here is your current :calendar_spiral: Vertretungsplan **(%s)**: \n```\n%s```" % (ctx.message.author.mention, datum, text))
...
@bot.command(name="set_type_timeout", description="set the timeout when typing in trio game")
@commands.guild_only()
@commands.has_permissions(manage_webhooks=True)
...
@bot.command(name="set_wait_timeout", description="set the timeout when waiting for a found combination in trio game")
@commands.guild_only()
@commands.has_permissions(manage_webhooks=True)
...
async def trio(ctx, incount):
    ...
    try:
        ...
        os.system('ffmpeg -i "concat:%s|%s|%s|%s|%s|%s|%s|%s|%s|%s" -acodec copy output.mp3' %
                  (music[0], music[1], music[2], music[3], music[4], music[5], music[6], music[7], music[8], music[9]))
    ...
    while count != 0:
        ...
        while such in suchs:
            such = random.randint(1, 50)
        suchs.append(such)
        search = ("🔎 SEARCHING FOR NUMBER: **%d**\n write \"s\" when you found a combination - you have **%s** seconds " % (such, waittimeout))
        ...
        def combocheck(m):
            ...
            if conti(c1_index, c2_index, c3_index):
                if feld[c1_index]*feld[c2_index]+feld[c3_index] == such:
                    antwort = "%s*%s+%s=%s" % (feld[c1_index],
                                               feld[c2_index], feld[c3_index], such)
                    return [True, antwort]
                if feld[c1_index]*feld[c2_index]-feld[c3_index] == such:
                    antwort = "%s*%s-%s=%s" % (feld[c1_index],
                                               feld[c2_index], feld[c3_index], such)
                ...
                if feld[c2_index]*feld[c3_index]+feld[c1_index] == such:
                    antwort = "%s*%s+%s=%s" % (feld[c2_index],
                                               feld[c3_index], feld[c1_index], such)
                    return [True, antwort]
                if feld[c2_index]*feld[c3_index]-feld[c1_index] == such:
                    antwort = "%s*%s-%s=%s" % (feld[c2_index],
                                               feld[c3_index], feld[c1_index], such)
                ...
                if feld[c1_index]*feld[c3_index]+feld[c2_index] == such:
                    antwort = "%s*%s+%s=%s" % (feld[c1_index],
                                               feld[c3_index], feld[c2_index], such)
                    return [True, antwort]
                if feld[c1_index]*feld[c3_index]-feld[c2_index] == such:
                    antwort = "%s*%s-%s=%s" % (feld[c1_index],
                                               feld[c3_index], feld[c2_index], such)
        ...
        try:
            ...
            try:
                await ctx.send("%s you have ⌚ **%s seconds** to send your solution... %s" % (msg.author.mention, type_timeout, multiple))
                msg = await bot.wait_for("message", check=trü, timeout=type_timeout)
            except asyncio.TimeoutError:
                await ctx.send("**%s 😔 no solution sent in time!**" % msg.author.mention)
            ...
            if fetz[0] and feett[0] and str(msg.content)[0:8] != str(msg.content)[9:]:
                if multiple == "":
                    await ctx.send("%s %s is **correct!** ✅" % (msg.author.mention, fetz[1]))
                else:
                    await ctx.send("%s %s is **correct!** ✅" % (msg.author.mention, (str(fetz[1])+" & "+str(feett[1])).replace("*", "\*")))

                if msg.author.id in ids:
                    punkte[ids.index(msg.author.id)] += 1
                    if punkte[ids.index(msg.author.id)] == trio_double:
                        await ctx.send("%s you need 2 combinations from now on!" % (msg.author.mention))
                else:
                    ids += [msg.author.id]
                    punkte += [1]
            else:
                await ctx.send("%s %s is **incorrect!** ❌" % (msg.author.mention, msg.content))
                if msg.author.id in ids:
                    punkte[ids.index(msg.author.id)] -= 1
                else:
                    ids += [msg.author.id]
    ...
    embed = discord.Embed(
        title=":checkered_flag:  :ribbon:  FINISHED  :ribbon:  :checkered_flag:", color=0xf1a90f)
...
async def mac(ctx, frange):
    ...
    try:
        ...
        await ctx.send("%s ❌ invalid range! (has to be 100,1000 or 10000)" % ctx.message.author.mention)
        return

    global mac_running
    if mac_running:
        await ctx.send("%s ❌ theres already a game running!" % ctx.message.author.mention)
        return
    ...
    while mac_running:
        ...
        try:
            ...
            if correct:
                if progress % 2 != 0:
                    text = (":white_square_button:   mirror: **%i**" %
                            chain[progress-1])
            ...
            try:
                ...
                if msg.content == str(chain[progress]):
                    ...
                    if msg.author.id in ids:
                        punkte[ids.index(msg.author.id)] = int(
                            punkte[ids.index(msg.author.id)])+1
                    else:
                        ids += [msg.author.id]
                        punkte += [1]
...
async def pppkkk(ctx):
    ...
    embed = discord.Embed(title="pi-pa-po-ki-ka", color=0xf1a90f,
                          description="pi: **2**\npa: **3**\n po: **5**\nki: **7**\nka: **11**")
    ...
    while pipapo:
        ...
        if captcha == 5 and progress != 1 and False:
            ...
            list = [x for _, x in sorted(
                zip(punkte, ids), key=lambda pair: pair[0])]
            list.reverse()
            cap_id = list[0]
            ...
            def cap(msg):
                return msg.channel == ctx.channel and msg.content == r and msg.author.id == cap_id

            num = num2words(r).replace("q", "𝕢").replace("w", "𝕨").replace("e", "𝕖").replace("r", "𝕣").replace(
                "t", "𝕥").replace("z", "𝕫").replace("u", "𝕦").replace("i", "𝕚").replace("o", "𝕠")
            num = num.replace("p", "𝕡").replace("a", "𝕒").replace("m", "𝕞").replace("n", "𝕟").replace(
                "b", "𝕓").replace("v", "𝕧").replace("c", "𝕔").replace("x", "𝕩").replace("y", "𝕪")
            num = num.replace("l", "𝕝").replace("k", "𝕜").replace("j", "𝕛").replace(
                "h", "𝕙").replace("g", "𝕘").replace("f", "𝕗").replace("d", "𝕕").replace("s", "𝕤")

            await ctx.send("<@%s> please prove that you are alive by typing %s as a number, you have 15 seconds" % (cap_id, num))
            try:
                await bot.wait_for('message', check=cap, timeout=15)
                await ctx.send("not a bot... ok....")
            except:
                await ctx.send("<@%s> you are now banned from this round!" % str(cap_id))
                ban.append(cap_id)
        ...
        try:
            ...
            if progress % 5 == 0:
                embed = discord.Embed(title="Number: %s" %
                                      str(progress), color=0xf1a90f)
...
async def help(ctx):
    ...
    **LGH Bot** by <@432116634807959554>
    """ % (prefix, prefix, prefix, prefix, s.mention, prefix, prefix, prefix, prefix, prefix, prefix)
...
async def suggest(ctx, *args):
    ...
    if str(ctx.message.author.id) in ban:
        await ctx.send("%s unfortunately you are banned from using this command" % ctx.message.author.mention)
        return
    ...
    embed = discord.Embed(title="New Suggestion",
                          description=sugg, color=0xf1a90f)
...
async def on_command_error(ctx, error):
    if error != TimeoutError:
        await ctx.send(("%s an error occured: %s") % (ctx.message.author.mention, str(error)))
